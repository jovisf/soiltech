...
export default async function globalSetup() {
  ...
  if (
    ...
    process.env.DATABASE_URL.includes('soiltech')
  ) {
    process.env.DATABASE_URL =
      'postgresql://soiltech:soiltech@localhost:5432/soiltech_e2e_test?schema=public';
  }
  process.env.NODE_ENV = 'test';
  console.log(
    'e2e globalSetup: process.env.DATABASE_URL set to',
    process.env.DATABASE_URL,
  );
